...
def trainClassifier(X, Y, weight, bias, mu):
    totalUpdates = 0;
    for row, y in zip(X, Y):
        eta = (mu - y*(weight.dot(row)))/(np.dot(row,row) + 1)
        prediction = (weight.dot(row) + bias);
        if y*prediction < mu:
            weight = weight + eta*y*row
            bias = bias + eta*y
        totalUpdates += 1
    return weight, bias, totalUpdates
    ...
    for mu in mus:
        ...
        for keepOut in range(5):
            ...
            for epoch in range(10):
                ...
                # Training the classifier.
                fWeight, fBias, updates = trainClassifier(Xtrain, Ytrain, fWeight, fBias, mu)
            accuracy = predict(Xcv, Ycv, fWeight, fBias)
            avgAccuracy += accuracy
        avgAccuracy = avgAccuracy / (5)
        muAccuracy.append(avgAccuracy)
        counter += 1
        # print("Eta accuracy: " + str(avgAccuracy))

    # Getting max accuracy:
    accIndex = np.argmax(muAccuracy)
    print('The best hyper-parameters are: ' + str(mus[accIndex]))
    print('The cross validation accuracy for the best hyperparameter is: ' + str(round(muAccuracy[accIndex], 2)) + '%')
    return mus[accIndex], muAccuracy[accIndex]

def main():

    #Choosing the hyperparameter
    mu, muAccuracy = chooseBestHyperParameter()
    ...
    for epoch in range(20):
        ...
        #fWeight, fBias, updates = trainClassifier(Xtrain, Ytrain, fWeight, fBias, eta)
        fWeight, fBias, updates = trainClassifier(Xtrain, Ytrain, fWeight, fBias, mu)
